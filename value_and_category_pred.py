...
def wordCount(data):
    ...
    for d in data:
        words = ''.join([c for c in d['question'].lower() + d['answer'].lower() if not c in punctuation]).split()
        for w in words: # and if w not in stopwords:
            word_count[w] += 1
    word_count = [(word_count[w], w) for w in word_count]
    word_count.sort()
    word_count.reverse()
    #print("total unique words: " + str(len(word_count)))
...
def categoryCount(data):
    ...
    #print("total unique categories: " + str(len(category_counts)))
...
print "Score of value prediction: {}".format(model.score(X_val, y_val))
...
print "Score of category prediction: {}".format(model.score(X_val2, y_val2))
